 ...
 *  - 44px 미만인 버튼 — 걸으면서 한 손으로 누르는 앱이다
 *  - 가로 스크롤이 생기는 화면
 *  - 런타임 오류와 콘솔 오류
 *
...
async function run(label,file,seed){
  ...
  for(const t of ['collect','plan','now','jp']){
    ...
    const c=await p.evaluate(()=>{
      ...
      // 44px 미만 버튼
      const small=[...document.querySelectorAll('.tab.on button, .tab.on .xaction')]
        .filter(e=>{const r=e.getBoundingClientRect();return r.height>0&&r.height<40;})
        .map(e=>e.textContent.trim().slice(0,14));
      return {contrast:[...new Set(out)],over:wide,small:[...new Set(small)]};
    });
    if(c.contrast.length||c.over||c.small.length)
      bad.push({빌드:label,탭:t,대비:c.contrast,가로넘침:c.over,작은버튼:c.small});
  }
