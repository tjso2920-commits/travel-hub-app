...
요청으로 직접 실행해 확인**하고 지적한 미리보기 서버 경로 버그를
수정했다. 이번에도 새 기능은 없다 — 전부 버그 수정과 문서·실행
검증이다.

| 항목 | 수정 위치 | 재현/검증 결과 | 완료 여부 | 상태 | 다음 행동(사용자) |
|---|---|---|---|---|---|
| R15-1 Google 기존 계정 연결 화면 완성 | `src/design/spots.js`, `server/auth.mjs` | 서버가 이미 돌려주던 409(ownership-verification-required) 거절을 받아 처리할 화면이 없던 것을 확인 — 안내→이메일 코드 발송→소유확인→기존 계정(저장 장소·이용권 유지)으로 로그인 완료까지, 취소·코드만료·틀린코드·재전송·베타 초대코드 입력까지 전부 화면으로 연결. Google idToken이 메모리에서만 오가고 localStorage 등에 저장되지 않음을 테스트로 확인(`server/test/google-auth.test.mjs`, `scripts/test-google-signin-configured.mjs`) | 완료 | `모의(mock)로만 검증`(실제 Google 계정 연결 아님) | GOOGLE_CLIENT_ID 발급 후 실제 Google 계정으로 확인 필요 |
| R15-2 미리보기 서버 루트 경로 정적 자산 404 버그 수정 | `scripts/serve.mjs` | ChatGPT가 `scripts/serve.mjs`를 직접 실행해 실제 HTTP로 재현: `GET /`는 200이지만 그 HTML이 상대경로로 부르는 `/spots.css`·`/spots.js` 등은 404(파일이 실제로는 `/design/` 아래 있는데 주소가 `/`라 브라우저가 엉뚱한 경로로 찾음). 파일 내용만 맞춰 돌려주는 방식으로는 못 고치므로 `/`와 `/design`을 `/design/`으로 302 리다이렉트해 주소와 실제 파일 위치 기준을 통일. CSS·JS·폰트(woff2)·이미지(webp)·manifest·아이콘·보조 페이지(landing/admin)·구버전 링크까지 전부 실제 HTTP 200 재확인, 인코딩된 경로 순회 시도(`%2e%2e`)는 여전히 404로 차단됨도 재확인 | 완료 | `실사용 가능` | 없음 |
| R15-3 문서에 적은 실행 방법을 실제로 검증 | (코드 변경 없음 — 실행 검증만) | `node server/index.mjs`(임시 테스트 DB)와 `node scripts/serve.mjs`를 실제로 띄운 뒤: 같은 주소에서 `/api/health` 중계 확인 → 테스트 이메일 어댑터로 로그인 코드 발급(테스트 DB에서 코드 직접 조회, 실제 이메일 미발송) → 로그인 → 장소 저장 → 로그아웃 → 새 코드로 재로그인(같은 계정, 데이터 보존) 확인 → API 서버를 완전히 재시작해도 이전 세션 토큰과 저장 데이터가 그대로 남음을 확인. 실제 계정·결제 데이터는 쓰지 않았고, 확인 후 프로세스 종료+임시 DB 삭제로 정리함. **실제 공급자(Resend/토스/Google/Places) 연결이 아니라 테스트 어댑터 기준 서버 로직 검증**이라는 점을 구분해 기록 | 완료 | `실사용 가능`(테스트 어댑터 기준) | 실제 공급자 키 연결은 3-0절 7단계대로 사용자가 직접 진행 |
| R15-4 "저장소에 배포 구성 없음"과 "사용자가 서버 없음" 혼동 정정 + PC 실행 안내 | `docs/OPERATIONS_SETUP.md` 0-0/0-3-1/0-3-2절 | 이전 라운드 문서가 "호스팅이 없다"는 저장소 확인 사실을 "사용자에게 서버가 없다"는 단정처럼 읽힐 수 있었던 표현을 정정. Windows/macOS/Linux별 실행 명령을 짧게 추가하고, 같은 Wi-Fi HTTP(제한된 확인)와 HTTPS 테스트 주소(Google 로그인·GPS·결제 복귀까지)를 확인 가능 범위가 다른 별개 단계로 명확히 구분, HTTPS를 만들려면 필요한 것(본인 도메인·서버·리버스 프록시)을 실행하지 않고 설명만 제시. GitHub Pages 같은 정적 전용 호스팅만으로는 백엔드 기능이 하나도 동작하지 않는다는 점을 명시적으로 경고 | 완료 | 문서 작업 | 0-3-1절 순서대로 본인 PC에서 먼저 실행 |

**아직 미검증(정직하게 남김, 이전 라운드와 동일)**: 실제 Resend·
토스페이먼츠·Google Places/Routes·Google 로그인 키 연결, 실제 Google
계정으로 R15-1 화면을 끝까지 통과하는 것, 실제 iPhone/Android 기기
확인, 실제 AI 공급자 청구 비용 대조 — 전부 사용자가 실제 키·기기로
진행해야 하는 다음 단계다.
...
**실기기 베타 확인 절차(15차 재정리 — 별도 기기 구매·유료 테스트
서비스 계약 없이 진행) — 아래 순서로, 1번(운영자 본인 iPhone)부터
먼저 끝까지 확인한 뒤에 3번(베타 참가자)으로 넘어간다:**

**1단계 — 운영자 본인의 iPhone(Safari)에서 전체 흐름 1회**
ZIP 가져오기 → 장소 검색·분류 확인 → 코스 생성 → 저장 → 로그아웃 →
재로그인(저장한 게 남아 있는지) → 일본어 "여기서 쓸 말" 듣기(🔊)·
글자 크게 보기 → 테스트 결제(완료 후 토스 쪽에서 취소 처리) 순서
그대로, 끊지 말고 한 번에 끝까지 이어서 확인한다. 중간에 막히는
지점이 있으면 그 화면 이름을 그대로 적어 둔다.

**2단계 — 한국에서 후쿠오카 위치로 가정한 테스트(가상위치 vs 실GPS
구분)**
실제로 후쿠오카에 가지 않고도 코스·거리순 로직을 확인하려면, 관리자
전용 API로 그 계정에 테스트 위치를 켠 뒤(`OPERATIONS_SETUP.md`
1-6b절, `/api/admin/test-access`) 화면에서 후쿠오카 좌표를 테스트
위치로 지정한다. **이때 화면 위쪽에 항상 "🧪 테스트 위치 사용 중:
(지역명) · 실제 위치 아님(수동 지정)" 배너가 떠 있어야 한다** —
이 배너가 안 뜨거나, 실제 GPS로 얻은 값처럼 보이면 그 자체가
버그다(가상위치와 실제 위치를 화면에서 구분 못 하게 되는 것이므로
반드시 배너로 구분돼야 함). 확인이 끝나면 배너의 "실제 위치로
되돌리기"로 반드시 끈다 — 켠 채로 실사용자 계정에 남기지 않는다.

**3단계 — 소수 베타 참가자(참가자 본인 기기, 5~10분 분량)**
참가자들이 실제로 쓰는 **Android Chrome·삼성 인터넷(Samsung
Internet)**에서, 위 1단계와 같은 흐름 중 아래 핵심만 추려서 확인해
달라고 요청한다:
1. ZIP/CSV로 저장 목록 가져오기 → 한글 파일명 포함 파일 선택이 됨
2. 검색 → 자동 분류 결과 확인 → 태그 하나 고쳐 저장됨
3. 거리순 정렬 옆 "내 위치 기준" 버튼 → 위치 권한 허용/거절 각각 확인(이건 실제 GPS다 — 2단계의 가상위치와 다른 경로)
4. 오늘 동선 담기 → 코스 만들기 → 결과 저장됨 → 로그아웃 후 재로그인해도 남아 있음
5. (Instagram/Threads에서 링크를 열었다면) 가져오기·로그인·결제 중 막히는 화면이 있는지, 있다면 "외부 브라우저로 열기" 안내가 뜨고 그 안내를 따라가면 실제로 이어갈 수 있는지(막다른 화면으로 끝나면 안 됨)
6. 문제가 있으면 프로필 → "불편함 보내기"로 바로 제출(화면 이름과 무엇을 하다 막혔는지만 적어도 충분 — 기기·브라우저 정보는 자동으로 같이 감)

**에뮬레이션 통과 ≠ 실기기 통과 — 반드시 구분해서 기록한다.** 이
세션이 지금까지 "통과"로 표시한 항목은 전부 Chromium 뷰포트
에뮬레이션이거나 모의(mock) 응답 기반 검증이다(R12-5 등) — 위 1~3단계처럼
**사람이 실제 iPhone·Android 기기를 손에 들고** 확인한 결과만 "실기기
확인 완료"라고 기록한다. 에뮬레이션 통과를 실기기 통과로 바꿔
보고하지 않는다. 새 기기를 구매하거나 유료 테스트 서비스와 계약하지
않는다 — 운영자 본인 기기 + 이미 모집한 소수 베타 참가자의 기기만
쓴다.
