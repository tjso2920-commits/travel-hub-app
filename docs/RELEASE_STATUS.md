...
## -18. 자동 분류 개선(2026-09-24, 가장 최신 — 여기부터 읽기)

**한 줄 결론**: 구글맵 목록에 식당·옷집·카페·관광지가 섞여 있어도, 가져오면 근거가 있는 만큼 자동으로 큰 분류·세부 분류가
붙고, 목록 화면에서 "쇼핑 → 의류"처럼 원하는 것만 볼 수 있다. 추가 API 호출·AI 호출은 없다. 가격·제공량·차감 정책·비용 한도·
운영 예산은 바꾸지 않았다. 공개 배포·실과금·master 병합은 하지 않았다(작업 브랜치 `design-integration`).

### 자동 분류 가능한 근거(우선순위)
1. **사용자가 직접 고른 분류·태그**(확정) — 어떤 자동 처리도 덮지 않는다.
2. **장소 확인 때 이미 받은 Google 업종**(`types`, `primaryType`) — 이미 앱에서 허용된 위치 확인에서 같이 오는 값만 쓴다.
   `primaryType`을 먼저 보되 구체 업종(`clothing_store`, `sushi_restaurant` 등)을 포괄 업종(`store`, `food`)보다 우선하고,
   `establishment`·`point_of_interest`처럼 업종을 말해 주지 않는 값은 무시한다. 국가와 무관하다(파리 옷가게·방콕 태국식당도 같은 표).
3. **장소 이름·사용자 메모의 분명한 업종 낱말**(예: 古着·스시·미술관·Cafe). 큰 분류 낱말이 없어도 세부 업종 낱말이 분명하면
   (예: "○○ 주판점") 그 세부 업종의 큰 분류를 쓴다.
4. 위 근거가 없으면 **미분류**.
- 큰 분류: 식당 · 카페·디저트 · 바·주점 · 쇼핑 · 관광·문화 · 휴식·미용 · 숙소 · 교통 · 약국·의료 · 미분류.
  세부 분류는 큰 분류에 연결돼 있다(예: 쇼핑 → 의류·신발·잡화·백화점·쇼핑몰·마트·편의점·주류샵·시장·서점). 예시 어휘이며
  일본 전용 목록이 아니다. 먹고 마시는 곳(식당·카페·바)끼리는 세부 업종이 겹칠 수 있다(야키토리+이자카야).

### 미분류로 남는 경우
- 이름에 업종 낱말이 없고(브랜드명만 — 예: "UNIQLO Ginza") 아직 위치 확인을 하지 않은 곳. 브랜드만 보고 업종을 단정하지 않는다.
- 위치 확인을 했어도 Google이 `establishment`·`point_of_interest`처럼 업종 없는 값만 준 곳.
- **목록 이름**("쇼핑", "여행 계획")은 업종 근거로 쓰지 않는다 — "쇼핑" 목록의 카페는 카페로 분류된다.
- **주소**도 근거로 쓰지 않는다(이번에 발견·수정: 주소 "東京都港区"의 港(항구) 때문에 카페가 교통으로 분류됐다).
- 미분류여도 가져오기·지도·코스·영업시간 등 다른 기능은 그대로 쓸 수 있고, 상세 화면에서 두 번 눌러 바로 고칠 수 있다.

### 기존 사용자 데이터 보존
- 저장값(`place.cat`의 한국어 문자열)은 바꾸지 않았다 — 화면 표시명만 따로 둔다(예: 저장값 "맛집·식당" → 표시 "식당",
  "사우나·온천"과 "마사지·스파"는 저장값 그대로 "휴식·미용"으로 묶어 보여 줌). 동기화·기존 테스트·예전 데이터와 그대로 호환.
- 사용자가 확정한 분류·태그, 일부러 비운 태그, 개인 태그, 목록 소속, 코스 연결은 그대로다. 자동 처리는 이미 있던 태그를
  지우지 않고 근거가 있는 것만 **덧붙인다**. 사용자가 큰 분류를 직접 바꿀 때만, 자동으로 붙었던 다른 분류의 세부 태그를 뗀다.
- 재가져오기가 장소 확인으로 받은 업종 분류를 이름 짐작으로 덮던 문제를 고쳤다.
- 기존 장소 중 사용자가 확정하지 않은 곳은 앱을 열 때 **이미 가진 근거만으로** 보완한다(새 조회 없음).
- 분류 근거(직접 지정 / 구글 지도 업종 / 이름으로 짐작 / 근거 부족)는 저장하지 않고 화면에서 계산해 보여 준다(데이터 변경 없음).
  정확도 백분율은 보여 주지 않는다.
- 공급자 데이터 보관 방식은 그대로다: 새 공급자 필드를 저장하지 않고, 기존 `confirmedTypes`에 `primaryType`을 맨 앞 순서로만 담는다.

### 추가 API 호출 여부
- **없음.** 분류만을 위한 Places 조회·요청 필드 확대·AI 호출을 하지 않는다. 가져오기·필터 선택·화면 열기로는 장소 확인·
  영업시간·경로·AI 분류 요청이 0건이고, 비용 원장 기록도 0건이다(아래 검증).
- 업종 보완은 사용자가 이미 하던 "위치 확인 → 맞아요" 때 받은 값으로만 한다.

### 실제 실행한 검증과 미검증
- 새 검사 `scripts/test-auto-classify.mjs`(헤드리스 Chromium + 테스트 모드 서버): 섞인 목록 분류, 해외 업종 매핑, 쇼핑→의류 필터
  (원본 유지), 구체/포괄 업종, 근거 없음 → 미분류, 사용자 수정 후 재가져오기·새로고침·다른 기기 동기화 유지, 비운 태그 유지,
  예전 저장값·태그·목록·코스 연결 보존, 저장 실패 시 실패 안내, 가져오기·필터로 유료 호출 0건·비용 원장 0건 — 전부 통과.
- 수정 전 코드로 같은 입력을 돌린 결과(재현): 港区 카페 → 교통, 古着 가게 → 미분류(태그 없음), `store`+`clothing_store` → 쇼핑(의류 태그 없음).
- 관련 기존 검사(분류·태그·가져오기·장소 확인·동기화·화면·디자인 통합·서버 53개)와 마지막 전체 검증 1회 — 아래 결과.
- **미검증**: 실제 Google 응답(이 작업은 테스트 어댑터만 썼다). Google 공식 문서 페이지는 이 작업 환경에서 막혀 있었다 —
  `types`/`primaryType` 응답 구조는 Google 공식 API 정의(place.proto), 기본 업종 이름은 Google 공식 클라이언트 목록으로 확인했고,
...
**한 줄 결론**: 추가로 재현된 경계 문제 3건을 먼저 재현한 뒤 고쳤고, 유료 "영업시간 조회 40회" 안을 지금 코드로 계산해
문서 맨 앞에 정리했다(운영 설정은 안 바꿈). 새 기능은 넣지 않았다. 다음은 사용자 PC 실사용 테스트다.
공개 배포·실과금·유료 계약·SNS 게시·master 병합은 하지 않았다(작업 브랜치 `design-integration`).

| # | 문제 | 재현(수정 전 실제 결과) | 수정 | 검증 |
|---|---|---|---|---|
| 1 | 되돌리기가 다른 기기의 최신 변경까지 지움 | `scripts/test-course-undo-conflicts.mjs`(Chromium 여러 기기, 실제 화면 버튼)를 수정 전 코드로: **15건 실패** — 다른 기기가 추가한 10-29·다른 여행 11-02가 서버에서 사라짐, 다른 기기가 고친 메모가 `old`로 되돌아감, 장소가 옮기기 전 상태로 덮임 | 되돌리기는 **그 편집이 바꾼 날짜만** 다룬다(편집 전·후 내용을 날짜별로 기록). 그 날짜가 편집 직후 모습 그대로일 때만 되돌리고, 그 사이 다른 기기가 바꿨으면 되돌리기 전체를 **보류**하고 알린다("다른 기기에서 ○월 ○일 일정이 바뀌어 되돌리지 않았어요"). 다른 날짜·다른 여행은 건드리지 않는다. 옛 스냅샷에 현재 버전을 붙여 통째로 덮던 방식은 없앴다 | 같은 테스트 전부 통과: 내 시간 변경 → 다른 기기 새 날짜 추가 → 되돌리기(10-29·11-02 보존, 내 편집만 취소), 날짜 이동 → 다른 기기가 출발/도착 날짜 수정 → 되돌리기(보류, 메모 보존, 장소 겹침·유실 없음), 새로고침·재로그인·다른 기기 조회. 기존 "새 날짜로 옮긴 뒤 되돌리면 그 날짜가 사라짐"(`test-course-undo-devices`)도 통과 |
| 1-부 | 같은 내용인데 키 순서만 달라 "바뀜"으로 오판 → 거짓 충돌·옛 내용 재업로드 | 위 테스트의 여행 시나리오에서 발견: 서버가 돌려준 코스는 `date`가 앞에 와 비교가 어긋나 버전이 불필요하게 오르고, 다른 기기가 옛 내용을 다시 올림 | 서버·앱의 코스 내용 비교와 충돌 재병합 비교를 키 순서와 무관하게 | 서버 테스트 추가(키 순서만 다른 재전송은 버전 그대로, 옛 기준이어도 내용 같으면 충돌 아님) |
| 2 | 24시간 영업인데 다음 날 특별 휴무를 넘는 체류를 "24시간 영업"으로 통과 | 확인 2026-09-22, 09-23 특별 휴무, 09-22 23:50 도착·40분 → `open-24h "24시간 영업"` | 도착한 구간에 이어지는 구간까지 이어 실제 닫히는 시각을 구한 뒤 체류와 비교 | 수정 후 `closes-during "도착 10분 뒤 다음날 00:00에 영업 종료(다음 날 특별 휴무)"`. 일반 24시간·특별 휴무 당일·전날 낮·전날 밤·휴무 다음 날·자정에 끊겨 이어지는 정규 영업 회귀 테스트 추가 |
| 3 | 이동시간 미확인인데 영업 판정은 "영업 중 도착"으로 확정 | 앞 구간 미확인 → 가장 이른 도착 11:13을 그대로 판정 → `open` | 미확인 구간부터 뒤 장소까지 "도착 시각 미확인"을 추적. 영업시간은 그대로 보여 주되 시각에 따라 달라지는 판정은 "도착 시각 미확인 — 방문 가능 여부 판단 보류", 도착 시각은 "도착 시각 미확인(빨라야 11:13)". 그날 휴무·폐업처럼 시각과 무관한 판정은 그대로 | 계산 테스트 + 실제 화면 테스트(`test-business-hours-ui` 8-2). 유료 재계산 요청 0건 |
| 4 | 유료 영업시간 40회 안 | — | `docs/BUSINESS_DECISIONS.md` 0절 맨 앞: 장소 수 vs 조회 횟수, 실패 차감·원가, 기존 약속 유지 상한(안 a 4,600원 / 안 b 4,800원), 하루 한도에 걸리는 경우를 **지금 코드로 돌린 결과**(`scripts/hours-plan-simulation.mjs`)로 정리. 설정·가격·예산 변경 없음 | 합성 데이터 시뮬레이션(실제 Google 0건) |

**검증 구분(과장 금지)**
- 화면 테스트는 원격 작업 환경의 헤드리스 Chromium(Linux, Node 22.22.2)이다 — Windows·iPhone·Android 실기기가 아니다.
  (독립 검토에서 핵심 검사 5종이 Node 24에서 통과했다는 보고는 검토자 환경의 결과다 — 이 세션에서 Node 24로 돌리지는 않았다.)
- Google(장소·경로·영업시간)·토스·Resend는 모의(테스트) 어댑터로만 확인했다. 실제 연결은 0건이다.

**아직 안 된 것 / 미검증**: 실제 Google·토스·Resend 호출, Windows·iPhone·Android 실기기, 운영 호스팅(https), 공식 가격 페이지 직접 열람,
무료 구간이 실제 결제계정에 적용되는지. 제휴 수익화는 완료가 아니다.

**정해 주실 것**: ① 유료 영업시간 40회 — 안 a(실패도 차감, 상한 4,600원) / 안 b(실패 미차감 + 재시도 4회, 상한 4,800원 + 코드 변경) /
보류(지금처럼 여유 안에서만). ② 그에 따른 월 운영 예산(동시 판매 7 → 5개). ③ 서비스 전체 하루 비용 3,000원 유지 여부. ④ 운영 호스팅.

**전체 회귀(최종, 고친 코드로 1회)**: 전부 통과, 실패 0건 — `scripts/verify.mjs` + 옛 앱 기능 검사(`scripts/test/run-all.mjs`) + 디자인 통합 검사 + 서버 테스트 53개 파일 + 새 앱 화면 테스트 37개 파일(`scripts/test-*.mjs`, 이번에 추가한 되돌리기 충돌 두 기기 검사 포함). 실행 환경: 원격 작업 환경(Linux, Node 22.22.2, 헤드리스 Chromium) — 실기기·Windows·실제 외부 서비스 아님.


## -16. 18차 재검토(38bab58 독립 재검토) 수정 요약 (2026-09-23)

**한 줄 결론**: 재검토가 지적한 5가지 오류를 전부 먼저 재현한 뒤 고쳤고, 고친 코드로 전체 회귀를 한 번 돌렸다.
새 기능은 넣지 않았다. 실제 Google·토스·Resend 연결과 실기기 확인은 아직이다 — 다음은 사용자 PC에서 켜서 확인할 차례다.
공개 배포·실과금·유료 계약·SNS 게시·master 병합은 하지 않았다(작업 브랜치 `design-integration`).

기능 이름은 **"날짜별 코스 생성·편집 + 영업시간 확인/경고"** 로 적는다(여러 날짜 자동 배치 기능이 아니다).

| # | 지적 | 재현(수정 전 실제 출력) | 수정 | 검증 |
|---|---|---|---|---|
| 1-A | 날짜 없는 24시간 current가 버려져 **휴무로 오판** | `test-hours-24h`, 확인 2026-09-22 → 09-23 12:00 방문: `closed-day "이 날은 휴무예요"`, 설명 "휴무" | current의 모양을 날짜형/상시(24h)/특별일만/없음으로 구분, 상시는 특별 휴무일이 아니면 24시간 | 수정 후 `open-24h "24시간 영업"`. 어댑터 current+regular 조합·특별 휴무·빈 current·범위 밖·잘린 구간·전날 이월·여러 날 영업 테스트 추가(`scripts/test-day-schedule.mjs`, 3개 시간대) |
| 1-B | 폐업인데 "정보 없음"이 먼저 표시 | `businessStatus: CLOSED_PERMANENTLY`, 시간표 없음 → `no-info "등록된 영업시간 정보가 없어요"` | 영업 상태(폐업·임시휴업)를 시간표 유무와 분리해 먼저 판정 | 수정 후 `closed-permanently "구글 지도에 폐업으로 표시된 곳이에요"`. 불확실한 경우를 휴무로 바꾸지 않음(종료 미확인 → "종료 시각은 확인 범위 밖") |
| 2 | 새 날짜로 옮기기 → 동기화 → **되돌리기** 해도 서버에 새 날짜 코스가 남음(레거시·여행 코스 둘 다) | 두 기기 화면 테스트를 수정 전 앱 코드로 실행: `FAIL 되돌리기 후 서버에서 10-07 사라짐 — 2026-10-05,2026-10-07`, `FAIL 새로고침 후 로컬에 10-07 없음`, `FAIL 재로그인 후…`, `FAIL 다른 기기에서도 10-07 없음…` | 되돌리기로 없어진 날짜는 **명시적 삭제 표시**(기준 버전 + 지울 당시 내용)로 올림. 배열에서 빠졌다는 이유로는 서버가 절대 안 지움. 지운 날짜 내용이 다른 기기에서 바뀌었으면 지우지 않고 되살림 | `scripts/test-course-undo-devices.mjs`(Chromium 3개 컨텍스트, 실제 버튼): 되돌리기 → 동기화 완료 → 새로고침 → 로그아웃·재로그인 → 다른 기기 조회까지 10-07 없음, 모르던 기기가 옛 목록을 올려도 되살아나지 않음 |
| 3 | 여행 메타가 같으면 날짜별 코스가 **버전 확인 없이 덮어써짐**, 코스만 바뀌면 버전도 안 오름 | 서버 테스트(`server/test/course-sync-versioning.test.mjs`)가 수정 전 서버 코드에서 실패(같은 날짜 옛 기준 저장이 `conflicts:[]`로 통과) | 날짜별 코스에 버전·삭제 표시 열 추가(자동 이전, 기존 데이터 유지). 메타 충돌 여부와 무관하게 날짜마다 기준 버전 확인, 같은 날짜 충돌은 서버 값 유지 + 충돌 보고 → 앱이 3-way 재병합 후 다시 올림. `PUT /api/trips/:id/courses`의 "전부 지우고 다시 넣기"도 같은 규칙으로 | 같은 날짜 동시 수정: 두 기기 수정 모두 보존(조용히 덮지 않음). 다른 날짜 동시 수정: 둘 다 서버에 남음. 계정 격리 유지 |
| 4 | 코스에서 빼기·옮기기 때 좌표가 없으면 **옛 구간 시간(B→C)이나 0분을 확정값처럼** 남김, 경도 검사 누락 | A 좌표 없음 → B 빼기: C `walk: 9`(옛 B→C) 그대로 / 도착 날짜 마지막 장소 좌표 없음 → 옮긴 곳 `walk: 0, walkEstimated: true` / `estimateWalk({lat:1,lng:NaN},…)` → `NaN` | 이동시간을 정할 수 없으면 비우고 "이동시간 미확인 · 재계산 필요"로 표시(합계에 넣지 않고 개수로 알림). 위도·경도 둘 다 검사. 유료 경로 재계산은 자동으로 하지 않음("새로 만들기"를 눌러야 코스 생성 1회 사용 — 기존 정책 그대로) | 같은 입력으로 수정 후 `walkUnknown: true`, `null`. 테스트 7건 추가(수정 전 코드에서 6건 실패 확인) |
| 5 | 영업시간 단가를 $35(49원)로 잡음 | 코드 기본 `COST_PLACES_DETAILS_ENTERPRISE_USD_PER_1000=35` → 49원 | 공식 $20/1,000(28원)으로 정정. 환율·요금 변동 여유는 단가에 섞지 않고 별도 값(`BUSINESS_HOURS_COST_BUFFER_RATIO` 15%, 여유 판정 때만) | 서버 테스트: 단가 28원, 여유 판정 32.2원, 무료 5곳 정확히 허용. 가격·기존 제공량·원가 상한·월 예산은 **안 바꿈**. 무료5/유료20/유료40 안은 `BUSINESS_DECISIONS` 0절 맨 앞 표(제안, 승인 전 미적용) |

**보존 확인(이번에 다시 돌린 기존 검사)**: 지도 링크 추가 버튼 잠김 수정, "/" → 새 앱, 결제 후 복귀, 서비스워커(오프라인 대표 주소) —
전부 아래 전체 회귀에 포함돼 통과.

**검증 구분(과장 금지)**
- "실제 Chromium"은 이 작업 환경의 헤드리스 Chromium이다 — iPhone·Android 실기기가 아니다.
- Google(장소·경로·영업시간)·토스·Resend는 **모의(테스트) 어댑터**로만 확인했다. 실제 연결은 0건이다.
- 공식 가격 페이지는 이 세션에서 직접 열어 보지 못했다 — 재검토에 제시된 공식 값($20/1,000, 월 1,000건 무료)을 그대로 썼다.
- 이번 수정은 이 세션 안에서 한 것이다. 특정 모델의 "최종검수 완료"라고 적지 않는다.

**아직 안 된 것 / 미검증**: 실제 Google·토스·Resend 호출, iPhone·Android 실기기, Windows에서 실행, Node 24에서 실행,
운영 호스팅(https 주소), 무료 구간이 실제 결제계정에 적용되는지. 제휴 수익화는 **완료가 아니다**(준비 코드만, 신청·계약 없음).

**정해 주실 것**: ① 영업시간 제공 안(1: 약속 없음 권장 / 2: 유료 20곳·상한 4,000원 / 3: 유료 40곳·상한 4,600원) —
`BUSINESS_DECISIONS` 0절. ② 운영 예산 — 같은 문서 0-5. ③ 운영 호스팅 — `OPERATIONS_SETUP` 0절.

**전체 회귀(최종, 고친 코드로 1회)**: 전부 통과 — `scripts/verify.mjs` + 옛 앱 기능 검사(`scripts/test/run-all.mjs`) + 디자인 통합 검사 + 서버 테스트 53개 파일(`npm run server-test-all`) + 새 앱 화면 테스트 36개 파일(`scripts/test-*.mjs`, 이번에 추가한 두 기기 되돌리기 검사 포함, 헤드리스 Chromium). 실패 0건. 실행 환경: 원격 작업 환경(Linux, Node 22.22.2) — 실기기·Windows·실제 외부 서비스 아님.


## -15. 18차 + 출시 전 최종검수 요약(2026-09-22)

**한 줄 결론**: 날짜별 일정(요일·시간대·출발 시각·방문 시각·날짜 옮기기·빼기)과 영업시간 확인을 새 앱에 붙였고,
최종검수에서 재현한 결함 9건을 고쳤다. 실제 Google·토스·실기기 확인은 아직이다 — 다음은 사용자 PC에서 켜서
Google 로그인부터 확인할 차례다(`docs/OPERATIONS_SETUP.md` 0절). 공개 배포·실과금·유료 계약·SNS 게시·master 병합은 하지 않았다.

| 항목 | 수정 위치 | 내용 | 완료 여부 | 검증 구분 | 다음 행동(사용자) |
|---|---|---|---|---|---|
| 링크 조회 중 입력을 B로 바꾸면 버튼이 "확인 중…"으로 잠김 | `src/design/spots.js` | A 응답을 버린 뒤에도 버튼을 되살리고 B를 그대로 저장 | 완료(재현→수정) | 실제 Chromium | 없음 |
| 영업시간 조회 서버 | `server/routes/business-hours.mjs`, `server/adapters/business-hours.mjs` | 위치 확인된 장소만·버튼 누를 때만·Place Details(별도 SKU)·기존 약속 예산 보호·동시 요청 병합·자동 재시도 없음·저장 안 함 | 완료 | 합성 데이터(모의 공급자) | 제공량 결정(`BUSINESS_DECISIONS` 0-4), 실제 키로 5곳 확인 |
| 날짜별 일정 화면 | `src/design/spots.js`, `src/design/day-schedule.js` | 날짜+요일·기준 시간대·출발 시각 변경·방문 시각 지정·머무는 시간·내 메모(제공자 정보와 분리)·다른 날짜로 옮기기(도착 날짜 기존 코스 보존)·코스에서 빼기·되돌리기·저장 실패 시 원상복구 | 완료 | 실제 Chromium | 폰에서 한 번 조작 |
| 영업시간 판정 | `src/design/day-schedule.js` | 정규/이번 주/특별일 구분, 범위 밖은 "정규 기준·방문 전 재확인", 브레이크·자정 넘김·24시간·휴무, 정보 없음·실패·한도 ≠ 휴무, 종료 ≠ 마지막 주문, 업종 추측 없음, 출처·확인 시각·구글 지도 버튼 | 완료 | 순수 계산(3개 시간대) + 실제 Chromium | 실제 가게 몇 곳과 구글 지도 비교 |
| 미래 날짜 코스가 "지금 시각" 출발로 계산됨 | `src/design/spots.js` | 출발 시각 입력 추가(오늘=지금, 다른 날=09:00 기본) | 완료(재현→수정) | 실제 Chromium | 없음 |
| 한국 시간대 기기에서 "+ 날짜 추가"가 같은 날짜를 제안 | `src/design/spots.js` | 날짜 계산을 기기 시간대와 무관하게 | 완료(재현→수정) | 실제 Chromium(Asia/Seoul) | 없음 |
| 장소 검색 결과가 DB에 무기한 남음("10분 캐시") | `server/routes/places.mjs` | 10분 지나면 삭제 | 완료(재현→수정) | 서버 테스트 | 없음 |
| 신규 판매 차단이 "503/결제 준비 실패"로만 보임 | `server/adapters/payment-toss.mjs`, `src/design/spots.js` | 이유·결제 안 됨·기존 이용자 영향 없음 안내 | 완료 | 서버 테스트 + 실제 Chromium | 없음 |
| 1명분 남았을 때 두 사람이 연달아 결제창을 열면 둘 다 통과 | `server/adapters/payment-toss.mjs` | 30분 안의 대기 주문도 1개분 예약 + 판정·기록 트랜잭션 | 완료(재현→수정) | 서버 테스트 | 없음 |
| 상품 조건을 바꾸면 이미 산 사람 제공량도 바뀜 | `server/db.mjs`, `server/entitlement-usage.mjs` | 주문 시점 제공량·원가 상한을 주문에 저장 | 완료(재현→수정) | 서버 테스트 | 없음 |
| 판매 화면에 옛 앱 입구 | `src/design/index.html` | 링크 제거(옛 앱 파일·데이터는 보존) | 완료 | — | 없음 |
| 대표 주소 → 새 앱 | `server/static-site.mjs`, `server/index.mjs` | `SERVE_STATIC=true`면 한 서버가 화면+API, "/"→새 앱, 경로 탈출 차단, 결제 복귀·공유 쿼리 유지 | 완료(코드) | 실제 HTTP | 운영 호스팅 결정(미정 → 배포 미검증) |
| 오프라인에서 대표 주소가 옛 앱으로 뜸 | `src/sw.js`(v58) | 루트가 어디로 갔는지 기억해 오프라인에도 새 앱 주소로 | 완료(재현→수정) | 실제 Chromium+실제 SW(연결 끊김 재현) | 없음 |
| SW 오프라인 검사가 실제로는 온라인으로 돌던 문제 | `scripts/test-service-worker-cache.mjs` | Playwright 오프라인 설정이 SW 요청을 못 막음 → 서버 연결 끊기로 재현 | 완료 | — | 없음 |
| 디자인 검사 간헐 실패 2건 | `scripts/design-integration-check.mjs` | file:// 상태 확인 요청·정렬 순서 의존 — 검사 환경 문제로 확인 후 보완 | 완료 | — | 없음 |
| 비용 원장·결과 집계 분리 | `server/cost-ledger.mjs`, `/api/admin/api-usage` | 검색·영업시간·경로 요청 수·비용·성공/실패/재시도 + 판매 가능 인원 | 완료 | 서버 테스트 | 관리자 토큰 설정 후 확인 |
| 내 PC 실행 준비 | `start-windows.cmd`, `scripts/start-local.mjs`, `backup-windows.cmd`, `.env.example` | 한 번에 켜기·기존 DB 자동 백업·이 PC 안에서만·비밀키 표시 안 함 | 완료 | Linux·Node 22.22.2 실행 | **Windows 실행은 미검증** — 직접 켜 보기 |
| 사업 숫자 정리 | `docs/BUSINESS_DECISIONS.md` 0절 | 영업시간 원가·선택지, 예산 4구분·설정안, 가격·공헌이익 계산식, 제휴 준비 | 완료 | 코드 설정값에서 계산 | 영업시간 안·예산 안 선택 |

**검증 구분(과장 금지)**: "실제 Chromium"은 이 작업 환경의 헤드리스 Chromium이다 — iPhone·Android 실기기가 아니다.
"합성 데이터"는 서버의 테스트 어댑터가 만든 가짜 영업시간이다 — 실제 Google 응답이 아니다. 실제 Google·토스·
...
ChatGPT의 3차 독립 검토에서 2차 수정 중 두 가지가 여전히(또는 일부만)
재현됐다고 지적했다 — 지도 링크 저장의 이름 확인 상태가 "새 저장
시도를 시작할 때만" 무효화돼 "버튼을 다시 안 누르고 입력만 바꾸는"
경로를 놓쳤던 것, 그리고 서비스워커가 "네비게이션이면 전부 앱 문서"로
너무 넓게 취급해 `/api/health` 같은 API를 직접 열어도 캐시가 오염될
수 있던 것(2차에서 쓰기 키는 고쳤지만 대상 범위 자체를 안 좁혔음).
둘 다 재현 후 수정했다. 승인 디자인·가격·이용권·비용 한도는 그대로다.

| 항목 | 수정 위치 | 내용 | 완료 여부 | 상태 | 다음 행동(사용자) |
|---|---|---|---|---|---|
| R17r3-1 pendingLink 무효화를 입력 변경 즉시로 확장 | `src/design/spots.js` | 실제 재현: ① A 링크 제출 → nameRequired로 이름칸 열림 → ② 입력칸만 B 링크로 바꾸고 ③ 버튼을 다시 안 누르고 "이 이름으로 담기"만 누르면 A 링크에 새 이름이 저장됐다 — 무효화가 "새 저장 시도 시작 시점"에만 있어 이 경로를 안 탔다. 이제 `#mapLinkInput`에 `oninput` 리스너를 달아 직접 입력·길게 눌러 붙여넣기 등 입력이 바뀌는 모든 경로에서 그 즉시 pendingLink와 이름 입력 UI를 무효화한다(붙여넣기 버튼처럼 스크립트로 값을 바꾸는 경로는 input 이벤트가 안 나므로 같은 무효화 함수를 명시적으로 호출). 추가로 "입력 버전"(inputVersion) 카운터를 두어, 진행 중인 요청이 응답을 받았을 때 그 사이 입력이 바뀌었으면(버튼을 다시 안 눌러도) 저장하거나 이름 확인 UI를 열지 않도록 대조한다. 이름 확정 시에도 pendingLink를 만든 시점의 계정·입력 버전과 지금이 같은지 한 번 더 방어적으로 확인한다 | 완료 | `실사용 가능` | 없음 |
| R17r3-2 서비스워커 앱 문서 캐시 대상 범위 축소 | `src/sw.js` | 실제 재현: `/api/health`(같은 origin의 JSON API)를 네비게이션으로 직접 열면(주소창에 직접 입력 등 실제로 가능) 이전 코드는 "네비게이션이면 전부 앱 문서" 취급해 그 JSON 응답을 문서 캐시에 그대로 기록했다. 이제 실제 두 앱의 진입 경로(`/`,`/index.html`,`/design`,`/design/`,`/design/index.html`)를 정확히 나열한 목록에 있을 때만 캐시 대상으로 삼고, 그 외 네비게이션(예: `/api/health`)은 캐시를 전혀 안 건드리는 일반 통과 경로로 넘긴다. 대상 경로라도 응답이 실패·리디렉션됨·다른 origin·Content-Type이 text/html이 아니면 캐시를 쓰지 않는다(캐시 우선 전략이라 이런 경우 화면 자체는 항상 마지막 정상 캐시를 그대로 보여준다 — 사용자에게 잘못된 내용이 새지 않음). CACHE 버전을 올려 이전에 생긴 오염 캐시를 정리한다 | 완료 | `실사용 가능` | 없음 |
| R17r3-3 검증 | `scripts/test-map-link-add.mjs`(2개 시나리오 추가), `scripts/test-service-worker-cache.mjs`(3개 시나리오 추가) | 지적된 재현 절차를 그대로 옮겨 먼저 실패를 확인(가드를 임시로 지워 실제로 실패하는 것 확인)한 뒤 수정 → 재검증했다. 관련 기존 회귀(`test-zip-import.mjs`, `test-consumer-flow-e2e.mjs`)도 재실행해 통과 확인 | 완료 | 검증 작업 | 없음 |

**검증 구분**: 위 재현·수정·재검증은 전부 실제 Chromium + 실제
서비스워커(localhost HTTP) 또는 실제 인메모리 테스트 서버로 한 것이다
(모의 공급자·모의 결제는 이 라운드에 없음). Android 실기기 설치·구글맵
공유 메뉴 수신, iOS Safari 실기기 확인은 여전히 미검증이다(17차부터
동일).

## -13. 17차 2차 독립검토 갱신 요약

ChatGPT의 2차 독립 검토가 지적한 3가지를 재현 후 수정했다 — 계정
전환·화면 전환 중 지도 링크 저장 응답의 경쟁 상태(진짜 데이터 오염
버그), 서비스워커 캐시 "쓰기" 경로가 여전히 옛 앱 키만 쓰던 버그(3절
지적으로 실제 발견 — 읽기는 17차에서 고쳤지만 쓰기를 놓쳤었다), 중복
장소 안내 문구의 잘못된 지시("기존 장소를 고쳐 새 장소로 구분"). 전부
`design-integration` 브랜치에서 작업했고, 승인된 디자인·가격·이용권
수량은 그대로다.

| 항목 | 수정 위치 | 내용 | 완료 여부 | 상태 | 다음 행동(사용자) |
|---|---|---|---|---|---|
| R17r2-1 지도 링크 저장의 계정전환·화면전환 응답 경쟁 수정 | `src/design/spots.js` | 실제 재현: 계정 A로 저장 요청 → 응답 전에 로그아웃→계정 B로 로그인(`sessionEpoch` 증가, 전역 `foodMap`이 B로 교체) → A 요청의 늦은 응답이 그대로 `daFinalizeMapLinkAdd`를 불러 B의 `foodMap`에 A의 장소가 섞여 들어갔다. 이제 시트를 여는 시점에 화면 버전(`_screenVersion`)을 기록해 두고, 저장을 시도하는 시점에 `sessionEpoch`와 이 시도만의 순번(`requestSeq`)을 추가로 기록한다. 응답이 돌아오면 셋 중 하나라도 바뀌었으면(계정 전환/다른 화면으로 이동/이 화면 안에서 더 최신 저장 시도가 이미 시작됨) 완전히 버리고 아무 것도 저장·표시하지 않는다(이미 사라졌을 수 있는 DOM도 안 건드림). 클립보드 읽기 응답도 같은 화면 검사를 거친다. "이름 확인 대기" 중 링크를 바꿔 다시 누르면 이전 `pendingLink`와 이름 입력칸을 그 즉시 무효화해, 응답 도착 순서가 꼬여도 엉뚱한 링크에 이름이 안 붙는다. 로그인 전 입력 → 로그인 후 자동 이어서 저장 흐름은 그대로 유지 | 완료 | `실사용 가능` | 없음 |
| R17r2-2 서비스워커 문서 캐시 "쓰기" 경로 버그 수정 | `src/sw.js` | 재현: 17차에서 문서 캐시 "읽기"는 요청 경로(docKey)로 옛 앱/디자인 앱을 구분하도록 고쳤지만, 배경 갱신 결과를 캐시에 "쓰는" 줄은 여전히 무조건 `'./index.html'`만 썼다 — 디자인 앱을 열어 보기만 해도(온라인 상태) 조용히 옛 앱의 오프라인 캐시가 디자인 앱 내용으로 덮어써지는 실제 데이터 오염이었다. 쓰기도 읽기와 같은 docKey를 쓰도록 고치고 `CACHE` 버전을 올려(이전 버전이 만든 오염 캐시를 자동 정리) 배포한다. 겸사겸사 디자인 앱이 이 SW의 "새 버전 배포됨" 메시지를 전혀 안 듣고 있던 것도 발견해, 기존 앱과 같은 알림(토스트)을 붙였다(문서 하나만 캐시할 뿐 스크립트·스타일까지 오프라인 지원하는 건 아니라는 점은 코드 주석과 이 문서에 정확히 남김) | 완료 | `실사용 가능` | 없음 |
| R17r2-3 중복 장소 안내 문구 수정 | `src/design/spots.js` | "이미 저장된 장소예요" 화면에서 "같은 이름의 다른 지점이라면 장소 상세에서 이름·주소를 고쳐 새 장소로 구분해 두세요"는 기존 장소를 마치 다른 지점인 것처럼 고치라는 잘못된 지시였다(기존 장소 편집은 새 지점 추가가 아니다). "다른 지점이면 그 지점의 구글맵 링크로 다시 추가하라"는 안내로 바꿨다 — `daMerge`가 검증된 식별자(placeId·URL·좌표)가 다르면 이름이 같아도 자동으로 합치지 않는 기존 원칙을 그대로 안내하는 문구다 | 완료 | `실사용 가능` | 없음 |
| R17r2-4 검증 | `scripts/test-map-link-add.mjs`(8개 단언 추가), `scripts/test-service-worker-cache.mjs`(신규, localhost 실 HTTP+실 서비스워커) | 지적된 3가지를 먼저 재현(가드를 임시로 지워 테스트가 실제로 실패하는 것 확인) → 수정 → 재검증(가드를 되살려 통과) 순서로 진행했다. 서비스워커는 file://로는 등록 자체가 안 돼(직전 라운드가 file://로만 보고 "실사용 가능"이라 적었던 게 부정확했음을 이번에 확인·정정) 새로 만든 경량 localhost HTTP 서버 위에서 실제 등록·캐시·오프라인·오류응답·공유쿼리까지 검증했다. 관련 기존 회귀(`test-zip-import.mjs`, `test-consumer-flow-e2e.mjs`)도 재실행해 통과 확인 | 완료 | 검증 작업 | 없음 |

**정직하게 남기는 한계**: "새 버전 배포됨" 알림(토스트)이 그 배포를
일으킨 바로 그 새로고침 자신에게 도착하는 타이밍은 브라우저의
클라이언트 식별 시점에 좌우돼 이 샌드박스에서 재현이 들쭉날쭉했다
(서비스워커 쪽은 알림을 정상적으로 보내는 것까지는 확인됨 — 옛 앱의
기존 동일 메커니즘도 같은 구조라 이번에 새로 생긴 문제는 아니다). 캐시
정확성(이번 라운드의 핵심 수정)은 반복 실행으로 확정적으로 확인됐다.
Android 실기기 설치·구글맵 공유 메뉴 수신은 여전히 미검증이다(17차와
동일 — iOS Safari는 설계상 미지원).

## -12. 17차 갱신 요약

"지도 링크로 한 곳만 추가"를 재사용·개선해 "빠른 장소 추가" 기능을
새로 만들었다. 새 서버 엔드포인트나 새 저장 로직을 만들지 않고, 기존
링크 해석(`/api/places/resolve-link`)·병합 엔진(`daMerge`)·오늘 동선
(`route` Set)·로그인 재개 패턴을 그대로 재사용했다. Web Share
Target(안드로이드 설치형 PWA에서 "공유 → 이 앱 선택")도 최소 구현
했다 — iOS Safari는 이 경로 자체가 없어(WebKit 미지원) 링크 붙여넣기가
모든 기기의 기본 흐름으로 남는다.

| 항목 | 수정 위치 | 내용 | 완료 여부 | 상태 | 다음 행동(사용자) |
|---|---|---|---|---|---|
| R17-1 daMerge 결과 항목 구분 | `src/design/import-adapter.js` | `daMerge`가 기존엔 `{added, updated, skipped, dupCandidates}` 개수만 돌려줘, 단건 추가에서 "이미 있던 곳"과 "재수입 지문(importFingerprint)으로 갱신된 곳"을 구분 못 했다. 입력 항목별 `{place, matchKind}`(`exact`/`already`/`new`/`skipped`)를 담은 `resultItems`를 추가했다(기존 반환값·기존 호출부 3곳은 그대로 — 새 필드를 안 쓰면 동작 그대로). "이름+링크" 공유 텍스트에서 URL과 이름 힌트를 분리하는 `daExtractMapsUrl`도 함께 추가(링크 해석 자체는 여전히 서버가 함) | 완료 | `실사용 가능` | 없음 |
| R17-2 "장소 추가" 시트 전면 개편 | `src/design/spots.js` | 예전엔 시트를 열기도 전에 로그인부터 막아 입력조차 못 했다. 이제 시트가 항상 먼저 열리고, 붙여넣기·입력이 끝난 뒤 "저장"을 누르는 시점에만 로그인을 요구하며, 그때 입력해 둔 내용을 그대로 들고 로그인 후 자동으로 이어서 저장까지 끝낸다(내용 유실 없음). 클립보드 읽기는 새 "붙여넣기" 버튼을 눌렀을 때만 시도하고(자동·반복 시도 금지), 권한 거부·미지원이면 길게 눌러 직접 붙여넣으라고 안내한다. 저장 결과는 `resultItems`의 matchKind로 정확히 갈린다 — 이미 있던 곳이면 "이미 저장된 장소예요"+"장소 보기"(중복 생성 대신 기존 장소로 이동), 새 곳이면 "저장했어요"+"오늘 동선에 담기"(기존 `route.add()`만 호출 — 코스 재계산·이용권 차감 전혀 없음). 저장 버튼은 `saving` 플래그로 응답 오기 전 중복 클릭을 막는다 | 완료 | `실사용 가능` | 없음 |
| R17-3 헤더 "+" 버튼을 빠른 추가로 분리 | `src/design/index.html`, `src/design/spots.js` | 헤더의 눈에 띄는 "+" 버튼과 하단 "가져오기 ↗" 배너가 둘 다 같은 `[data-add]`로 Takeout/CSV/ZIP 전체 가져오기를 열었다. 이제 헤더 버튼만 `data-quick-add`로 분리해 새 빠른 추가 시트로 곧장 연결하고, 하단 배너는 그대로 전체 가져오기(`add()`)를 유지한다 — 기존 큰 가져오기 흐름은 그대로 있고, 한두 곳만 더 담고 싶을 때 쓰는 진입점이 새로 분명해졌다 | 완료 | `실사용 가능` | 없음 |
| R17-4 Web Share Target 최소 구현 | `src/design/manifest.webmanifest`(신규), `src/design/index.html`, `src/sw.js`, `src/design/spots.js` | 디자인 앱에 자체 매니페스트(`share_target`, GET, `title`/`text`/`url`)를 추가하고 기존 `src/sw.js`를 부모 스코프(`../sw.js`)로 재사용해 등록했다(새 서비스워커 파일은 안 만듦). **재사용 중 실제 버그 발견**: 기존 sw.js의 문서 캐시 응답이 요청 경로와 무관하게 항상 `./index.html`(옛 앱)만 돌려주고 있었다 — 그대로 디자인 앱까지 이 SW 스코프에 넣었다면 디자인 앱 새로고침 시 엉뚱한 화면이 뜨는 사고가 났을 것이다. 요청 경로(`/design/` 포함 여부)로 캐시 키를 고르도록 고쳤다(CACHE 버전도 올림). 부팅 시 `?title=&text=&url=` 쿼리를 읽어 빠른 추가 시트를 미리 채우고 쿼리스트링은 지운다 — 자동 저장은 안 하고 "저장"은 사람이 직접 누른다 | 완료(모의 검증까지) | `실사용 가능(안드로이드), 미지원(iOS Safari)` | 아래 "실기기 확인 필요" 참고 |
| R17-5 검증 | `scripts/test-map-link-add.mjs` | 기존 파일(7개 시나리오)을 확장해 총 34개 단언 — 로그인 전 시트 오픈, 저장 시점 로그인 요구+내용 보존+자동 이어서 저장, 이미 저장된 장소/신규 장소 결과 화면 분기, "이름+링크" 텍스트 파싱, 클립보드 성공/거부, 빠른 연속 클릭 중복 방지, 느린 네트워크+실패 후 재시도, Web Share Target 쿼리 파싱까지 — 전부 통과. 관련 기존 회귀(`test-zip-import.mjs`, `test-consumer-flow-e2e.mjs`)도 재실행해 전부 통과 확인. `design-integration-check.mjs`의 "지도 링크가 실제 저장 URL을 그대로 씀" 1건은 이번 변경 전 코드에서도 재현되는 기존 타이밍 의존 flaky 테스트로 확인됨(이번 변경과 무관 — 반복 실행 시 무변경 코드에서도 통과/실패가 갈림) | 완료 | 검증 작업 | 없음 |

**이미 되던 것**: 지도 링크 해석(짧은 링크·긴 링크·확정좌표 `!3d!4d`
구분), 중복 판정·병합(`daMerge`의 식별자 우선순위), 저장 실패 시
롤백, 로그인 재개(`showLoginSheet(cb)`) 패턴, 계정 동기화
(`daSyncPushSafe`) — 전부 기존 코드를 그대로 재사용했을 뿐 새로
만들지 않았다.

**이번에 새로 구현한 것**: 시트 오픈 시점과 로그인 요구 시점 분리(내용
보존), 붙여넣기 버튼(클립보드), "이름+링크" 텍스트 파싱, 저장 결과의
"이미 저장됨/신규" 분기 화면, 헤더 버튼 역할 분리, Web Share Target
매니페스트·SW 등록·쿼리 파싱, sw.js의 문서 캐시 경로 버그 수정.

**실기기 확인이 반드시 필요한 것(이 세션은 절대 확인 불가)**:
- 실제 HTTPS로 배포하고 안드로이드 크롬에서 이 PWA를 "홈 화면에 설치"한
  뒤, 구글맵 앱에서 실제로 "공유 → (설치된 이 앱 이름) 선택"이 앱 목록에
  뜨는지, 선택하면 실제로 이 앱이 열리고 쿼리스트링이 정확히 붙어 오는지.
  이 세션은 `file://`로만 구동했고 실제 설치·공유 메뉴 자체가 없어 그
  경로를 물리적으로 탈 수 없다(쿼리스트링을 직접 붙여 흉내만 냄).
- iOS Safari(홈 화면 추가 포함)에서 공유 메뉴에 이 앱이 뜨지 않는다는
  것 자체는 WebKit의 알려진 제약이라 설계상 확실하지만, 그 상태에서도
  "공유 → 링크 복사 → 이 앱 열어 붙여넣기" 기본 흐름이 실제 아이폰에서
  매끄러운지(길게 눌러 붙여넣기 동작, 클립보드 권한 프롬프트 문구 등)는
  실기기로 봐야 한다.
- 클립보드 자동 읽기(`navigator.clipboard.readText`)의 실제 권한
  프롬프트 문구·거부 시 브라우저 UX는 기기·브라우저마다 달라 실기기
  확인이 필요하다(이 세션은 스텁으로만 성공/거부를 흉내 냄).
- 실제 계정으로 폰-PC 간 동기화 후 새로 담은 장소가 다른 기기에도
  보이는지(서버 동기화 엔드포인트 자체는 기존 회귀로 확인됐지만, 두
  개의 실제 기기로 보는 것은 별개다).

가격·이용권 수량은 이번 라운드에서도 바꾸지 않았다. 공개 배포·실제
결제·`master` 병합은 하지 않았다.

## -11. 16차 4차 재검토 갱신 요약

ChatGPT의 4차 독립 검토가 지적한 마지막 남은 화면 전환 보호 1건을
수정했다. 저장 실패 재차감 방지·통합 문서 구성은 이미 확인됨. 이번
수정을 끝으로 자전거 안내 기능 개발은 마무리하고, 다음은 실제 연결
단계(Google 로그인 → 실제 장소·경로 소량 확인 → 테스트 결제 →
PC·모바일 동일 계정 확인)로 넘어간다.

| 항목 | 수정 위치 | 내용 | 완료 여부 | 상태 | 다음 행동(사용자) |
|---|---|---|---|---|---|
| R16r4-1 화면 전환 보호를 공통 화면 버전 기준으로 재설계 | `src/design/spots.js`, `src/design/bike-ports.js` | 실제 재현: 예전 `_bikeInteractionToken`은 이 파일(bike-ports.js) 안에서만 올라가, 사용자가 자전거와 **무관한** 일반 장소 상세·프로필로 이동해도(둘 다 spots.js의 공통 `open()`을 쓴다) 전혀 안 올라 늦게 온 자전거 응답이 그 화면을 덮어썼다. 이제 spots.js의 `open()` 안에서 직접 올리는 `_screenVersion`(모든 화면 전환의 공통 관문)을 그대로 쓴다 — 자전거 화면이든 아니든 어떤 화면이 새로 열려도 값이 달라진다. 처음엔 `daOpenBikeGuide`의 status 대기 지점에도 (버전+`sheet.open`) 둘 다 확인하도록 넣었으나, "출발지 없음 안내→직접 입력 시트를 close()로 닫고 곧장 같은 흐름을 이어가는" 정상 경로에서 `async` 함수 특유의 마이크로태스크 지연 때문에 그 시점의 `sheet.open`이 일시적으로 false로 보여 정상 흐름을 오탐 처리하는 버그가 실제로 재현돼, 그 지점만 버전 단독 확인(`daBikeVersionStale`)으로 바꿨다(후보 조회·안내 생성은 자기 로딩 화면을 먼저 그린 뒤에 값을 기억하므로 이 문제가 없어 기존대로 버전+`sheet.open`을 함께 봄) | 완료 | `실사용 가능` | 없음 |
| R16r4-2 검증 | 본 문서 | Chromium E2E에 이번 재현 시나리오 4개(후보 조회 중 일반 장소 상세 열기, 후보 조회 중 프로필 열기, 시트를 닫은 뒤 다른 화면을 열어도 이전 응답이 안 덮음, `daOpenBikeGuide`의 status 대기 중 화면 전환) 추가 — 전부 통과. 정상 GPS/직접 입력→자전거 안내 복귀 흐름(1~9절)과 저장 실패 복구 흐름(13·14절)도 이번 수정으로 깨지지 않았음을 재확인. 공용 코드(`open()`)를 다시 건드렸으므로 관련 기존 회귀(위치 옵트인·테스트 위치 모드·test-access·서버 3개)도 재실행 — 전부 통과 | 완료 | 검증 작업 | 없음 |

**아직 미검증(정직하게 남김)**: 이 세션의 아웃바운드 네트워크가 막혀
있어 Google Routes BICYCLE 모드를 실제 공급자로 확인 못 함(모의
시뮬레이션까지만), 실제 차리차리 반납 포트 운영 화면·실기기 확인,
실제 후쿠오카 현지 이동 확인, 스크래핑 데이터의 상업적 재사용 조건.
가격·이용권 수량은 이번 라운드에서도 바꾸지 않았다.

## -10. 16차 3차 재검토 갱신 요약

ChatGPT의 독립 검토에서 길찾기 링크·미완성 안내 미차감·비용 통합검사·
서버 접근 제한 4가지는 확인됐고, 남은 2가지 문제만 수정했다. 이번
수정을 끝으로 자전거 안내 기능 개발은 마무리하고, 다음 라운드부터는
실제 연결(Google 로그인 → 실제 장소·경로 소량 확인 → 테스트 결제 →
PC·모바일 동일 계정 확인) 단계로 넘어간다.

| 항목 | 수정 위치 | 내용 | 완료 여부 | 상태 | 다음 행동(사용자) |
|---|---|---|---|---|---|
| R16r3-1 저장 실패 후 새로고침 시 중복 차감 방지 | `src/design/bike-ports.js` | 실제 재현: `saveFoodMap`이 실패하도록 만든 뒤 같은 목적지·포트·출발지로 요청하고 새로고침 후 재시도하면, 요청 전 저장을 확인 안 하던 예전 코드는 새 idempotencyKey로 나가 서버가 새 작업으로 처리했다(주석의 "이미 성공이 카운트돼 재차감 안 됨"은 이 경로에서는 사실이 아니었다). 이제 **요청 전 저장이 실제로 성공했을 때만** 유료 요청을 보낸다 — 실패하면 저장 문제를 안내하고 공급자 요청 자체를 하지 않는다. 성공 후 결과 저장이 실패하는 경우엔 요청 키·서명(`requestKey`/`requestSignature` — 예전 `pendingKey`/`pendingSignature`라는 오해를 부르던 이름에서 변경, "성공해도 지우지 않는다"는 의미를 이름에 반영)을 지우지 않고 그대로 들고 있어, 새로고침 후 같은 목적지·포트로 다시 열면 서버가 재생(replay)으로만 응답해 재계산·재차감이 없다. 반납 완료·초기화 같은 무료 로컬 상태 전환도 저장 실패를 감추지 않고 안내한다 | 완료 | `실사용 가능` | 없음 |
| R16r3-2 후보 목록·상태 조회의 늦은 응답 보호 | `src/design/bike-ports.js`, `src/design/spots.js` | 실제 재현: 근접 포트 조회·활성 지역 상태 조회에는 안내 생성에만 있던 계정 전환(sessionEpoch) 검사가 없어, 계정 전환 중 응답이 오면 이전 계정 데이터가 새 계정 화면에 반영되거나 상태 캐시를 오염시킬 수 있었다. 두 조회 모두에 epoch 검사를 추가했고, "같은 계정이라도 다른 목적지를 열었거나 시트를 닫은 뒤 응답이 화면을 덮지 않게"는 새 `_bikeInteractionToken`(이 화면·요청이 아직 유효한지의 기준)으로 막는다. 처음엔 `<dialog>`의 네이티브 close 이벤트로 이를 구현했으나(phrasebook.js와 같은 패턴 시도), close 이벤트가 스펙상 비동기로 발화돼 "출발지 없음→직접 입력을 close()로 넘기고 즉시 같은 흐름을 이어가는" 이 화면의 정상 흐름에서 오탐(방금 정상적으로 이어진 다음 화면 요청을 잘못 무효 처리)이 실제로 재현돼, 동기 속성인 `sheet.open` 확인으로 바꿔 고쳤다 | 완료 | `실사용 가능` | 없음 |
| R16r3-3 운영 참고 — test_access 공유 명시 | `docs/OPERATIONS_SETUP.md` | 실제 포트 데이터 열람 권한이 새 승인 플래그 없이 기존 계정별 `test_access`(관리자 승인)를 그대로 공유한다는 점, 그래서 상업적 재사용 조건이 확인되기 전까지는 외부 베타 참가자 계정에 `test_access`를 켜지 않는다는 운영 조건을 명시적으로 문서화 | 완료 | 문서 작업 | 없음 |
| R16r3-4 검증 | 본 문서 | Chromium E2E(`test-bike-ports.mjs`)에 새 시나리오 6개(요청 전 저장 실패→공급자 호출 0회, 결과 저장 실패→새로고침→같은 키로 복구→재차감 없음, 목적지 A→B 전환 시 A 응답이 B를 안 덮음, 계정 전환 중 상태 조회 캐시 오염 방지) 추가, 예전 저장 실패 시나리오는 새 동작과 모순돼 새 시나리오로 대체. 서버 쪽은 이번에 변경이 없어 기존 3개 테스트만 재확인, 공용 코드를 다시 건드렸으므로 관련 기존 테스트(`test-location-optin.mjs`, `test-test-location-mode.mjs`, `test-access.test.mjs`)도 재실행 — 전부 통과 | 완료 | 검증 작업 | 없음 |
...
ChatGPT가 16차 결과물을 검토하고 5가지 문제를 지적했다 — 기능 확장
없이 전부 수정·검증했다. 새 지역·새 화면은 추가하지 않았다.

| 항목 | 수정 위치 | 내용 | 완료 여부 | 상태 | 다음 행동(사용자) |
|---|---|---|---|---|---|
| R16r-1 실제 길찾기 링크 추가 | `src/design/bike-ports.js` | 안내 화면(자전거+도보 구간)과 반납 완료 화면(도보 구간) 각각에 실제 출발·도착 좌표·이동수단이 담긴 구글맵 길찾기 버튼을 새로 추가 — 자전거 구간이 미지원/실패해도 도보 경로로 바꿔치기하거나 숫자를 만들지 않고, 링크만 항상 제공(링크는 숫자를 주장하지 않으므로 원칙 위반이 아님). "공식 지도/앱 열기"라는 표현이 앱을 직접 실행하는 것처럼 읽힐 수 있어 "공식 웹지도에서 보기"로 정정(실제로는 항상 표준 웹 링크였음 — 커스텀 앱 스킴 아님). Chromium으로 좌표·이동수단이 정확히 들어가는지 실제 확인 | 완료 | `실사용 가능` | 없음 |
| R16r-2 불완전한 안내의 이용 횟수 차감 버그 수정 | `server/routes/bike-ports.mjs`, `server/adapters/routing.mjs` | 예전엔 도보 구간 성공만으로 차감 여부를 판정해 자전거가 실패해도 이용권이 깎였다. 이제 "완전한 성공"을 두 구간 모두 실제 성공(`bikeLeg.routedReal && walkLeg.routedReal`)으로 재정의 — 자전거 실패+도보 성공처럼 일부만 성공하면 미차감. "다른 포트로 바꾸기"는 실행 전에 추가 차감 가능성을 안내 문구로 명시. 서버 테스트로 이 경계 시나리오(자전거만 실패)를 재현해 미차감을 확인(`bike-ports-bicycle-fallback.test.mjs` 신규 시나리오, 테스트 전용 `bike-fail-walk-success` 강제값 추가) | 완료 | `실사용 가능` | 없음 |
| R16r-3 두 구간 전체 비용을 호출 전에 함께 검사 | `server/adapters/routing.mjs`(신규 `computeBikeGuideRoutes`) | 예전엔 자전거·도보 구간이 각자 `chargeCostBatch`를 불러, 자전거만 예산을 통과해 먼저 과금·호출되고 도보에서 비로소 예산이 모자란 절반짜리 상태가 가능했다. 이제 두 구간의 세그먼트 계획을 먼저 다 세운 뒤 하나의 트랜잭션으로 예산을 확인·기록 — 하나라도 모자라면 자전거·도보 어느 쪽도 실제 공급자 호출을 안 한다(all-or-nothing). 개별 `computeBicycleRoute`는 제거(유일한 호출부가 옮겨감), 기존 `computeWalkingRoute`(코스 생성이 쓰는 별개 기능)는 손대지 않음. "자전거 세그먼트 하나만 보면 통과하지만 합계는 부족" 시나리오를 실제 HTTP+모의 fetch로 재현해 공급자 호출 0회·원장 기록 0건을 확인(`bike-ports-combined-budget.test.mjs` 신규) | 완료 | `실사용 가능` | 없음 |
| R16r-4 응답 유실·저장 실패·계정 전환 보호 | `src/design/bike-ports.js`, `src/design/spots.js` | 예전엔 안내 요청마다 새 idempotencyKey를 만들어, 응답이 유실된 뒤 재시도하면 서버는 같은 작업으로 인식 못 했다. 이제 요청 직전에 포트·출발지·목적지 서명과 idempotencyKey를 계정별 저장 데이터에 먼저 적어 두고, 같은 서명으로 재시도하면 그 키를 재사용(서버가 재생만 하고 재차감 안 함) — 포트·출발지·목적지가 실제로 바뀌었을 때만 새 작업으로 취급. `saveFoodMap` 반환값을 확인해 저장 실패를 성공처럼 감추지 않되, 이미 만든 결과는 화면에 그대로 보여줌(낭비 안 함). spots.js의 기존 `sessionEpoch` 메커니즘을 재사용해, 요청 중 로그아웃/계정 전환이 일어나면 늦게 도착한 응답을 조용히 버림(다음 계정 화면·저장소에 안 새어나감). 상태 캐시(활성 지역 목록)는 로그아웃 시 초기화하고 항상 현재 토큰으로 다시 조회. 저장한 안내에 목적지 좌표를 함께 보존해, 좌표가 바뀌면 예전 안내를 재사용하지 않음. Chromium으로 응답 유실→재시도(같은 idempotencyKey, 재차감 없음), 저장 실패(안내는 그대로 표시+정직한 안내), 요청 중 로그아웃(저장소 미오염) 전부 실제 재현·확인 | 완료 | `실사용 가능` | 없음 |
| R16r-5 실제 포트 데이터의 외부 노출 제한 | `server/routes/bike-ports.mjs` | 예전엔 로그인만 하면 어떤 계정이든 실제 포트 목록·건수를 볼 수 있었다(승인 없이). 새 승인 플래그를 만들지 않고 10차가 이미 만든 계정별 `test_access`(관리자 승인)를 재사용 — 승인 안 된 계정은 지역이 활성이어도 포트 건수·근접 목록이 항상 빈 값/0으로 오고(200 응답, 인증된 API 직접 호출로도 우회 불가), 화면은 기존 "포트 후보를 찾지 못했어요 → 공식 웹지도" 대체 화면으로 자연스럽게 이어짐. 저장 결과 재조회(멱등키 재생)도 같은 검사를 통과해야만 허용 — 승인이 나중에 거둬지면 예전 성공 결과도 다시 못 봄. 서버 테스트로 일반 계정 차단·승인 계정 정상 동작·승인 철회 후 재생 차단을 재현·확인, 실 스크래핑 데이터는 이번에도 저장소·테스트 픽스처·전달 ZIP 어디에도 없음(합성 좌표만 사용) | 완료 | `실사용 가능` | 상업적 재사용 조건이 확인되면 승인 대상을 넓힐지 결정 필요 |
| R16r-6 검증 | 본 문서 | 신규/변경 서버 테스트 3개(`bike-ports.test.mjs`, `bike-ports-bicycle-fallback.test.mjs`, `bike-ports-combined-budget.test.mjs` 신규) + Chromium E2E 1개(`test-bike-ports.mjs`, 12개 시나리오로 확장) 전부 실행해 통과 확인. 이번에 공용 코드(`daLogout`, `daFinishLogin`)를 다시 건드렸으므로 무관한 전체 회귀 대신 관련 기존 테스트만 재실행(`test-location-optin.mjs`, `test-test-location-mode.mjs`, `test-access.test.mjs`, `provider-contracts.test.mjs`, `routes-waypoint-limits.test.mjs`, `cost-budget-allornothing.test.mjs`, `reliability-and-cost.test.mjs`) — 전부 통과 | 완료 | 검증 작업 | 없음 |

**아직 미검증(정직하게 남김, 15차와 동일)**: 이 세션의 아웃바운드
네트워크가 막혀 있어 Google Routes BICYCLE 모드를 실제 공급자로 확인
못 함(모의 시뮬레이션·`fetch` 모의만 검증), 실제 차리차리 반납 포트
운영 화면·실기기 확인, 실제 후쿠오카 현지에서 자전거로 이동하며 안내가
맞는지 확인, 스크래핑 데이터의 상업적 재사용 조건(운영자 확인 전) —
전부 사용자가 실제 기기·현지·계약 확인으로 진행해야 하는 다음 단계다.
가격(9,900원/30일)·이용권 수량은 이번 라운드에서도 바꾸지 않았다.

## -8. 16차 갱신 요약

사용자가 "차리차리" 자전거 공유 반납 포트 안내 신규 기능 명세서
(00_READ_FIRST_CLAUDE.md, 이전 차리차리 전달문 대체)와 실제 스크래핑된
후쿠오카 포트 데이터를 전달하며 구현·검증을 요청했다. **이 기능은
차리차리 앱을 대체하지 않는다** — 대여/잠금해제/실시간 반납상태/이용
종료 확인은 전부 차리차리 공식 앱에서만 하고, 이 기능은 "목적지 근처
반납 포트까지 어떻게 가면 좋을지"만 보조한다.

| 항목 | 수정 위치 | 재현/검증 결과 | 완료 여부 | 상태 | 다음 행동(사용자) |
|---|---|---|---|---|---|
| R16-1 공급자→지역→포트 레지스트리 신규 구현 | `server/bike-share-providers.mjs`(신규) | "FUK/후쿠오카"를 코드 곳곳에 하드코딩하지 말라는 명세 요구대로, 활성화된 지역만 등록하는 단일 레지스트리를 신규로 만들었다. 현재 `charichari`/`FUK` 한 곳만 `active:true`로 등록, 다른 지역은 추측으로 지원 표시하지 않음 | 완료 | `실사용 가능` | 새 지역 추가 시 이 파일에만 등록하면 됨 |
| R16-2 자전거 구간 라우팅 어댑터 신규 구현(실패 시 숫자 절대 생성 금지) | `server/adapters/routing.mjs` | 기존 도보 어댑터는 실패해도 정직한 추정 숫자를 보여주는 반면(기존 원칙 유지), 신규 자전거 구간은 명세가 요구한 대로 **실패/미지원 시 거리·시간 숫자를 절대 만들지 않고** 대신 사유 문자열만 반환 — 화면은 이걸로 "외부 지도에서 확인" 링크로 대체한다. 서버 테스트로 성공/실패 양쪽 경로 모두 재현·확인(`server/test/bike-ports.test.mjs`, `server/test/bike-ports-bicycle-fallback.test.mjs` 신규) — 실패 응답에 distanceMeters·seconds 필드 자체가 없음을 명시적으로 확인 | 완료 | `실사용 가능`(로직) | 이 세션은 아웃바운드 네트워크가 막혀 있어 Google Routes BICYCLE 모드 자체를 실제 공급자로는 확인 못 함(모의 시뮬레이션까지만) |
| R16-3 근접 포트 조회 + 안내 생성 라우트 신규 구현 | `server/routes/bike-ports.mjs`(신규), `server/index.mjs`, `server/db.mjs`(`bike_share_ports`/`bike_share_import_meta` 표 신규) | 후보 목록(`/api/bike-ports/nearby`)은 무료 — DB·직선거리 계산만 쓰고 AI·Places·지오코딩은 전혀 안 씀(좌표를 이미 알고 있으므로), 목적지 기준 직선거리 오름차순 정렬(화면 문구도 "최단시간"이 아니라 "직선거리"로 명시), capacity(도크 수용량)를 실시간 가용대수처럼 보이지 않게 문구 분리. 안내 생성(`/api/bike-ports/guide`)은 기존 `course-generation.mjs`의 멱등키+계정별 잠금+이용권 차감 패턴을 그대로 재사용해, 자전거+도보 두 구간을 만들어도 **논리적으로 코스 생성 1회**로만 차감되고 같은 멱등키 재요청은 재생만 될 뿐 다시 차감 안 됨을 서버 테스트로 확인. 선택한 포트 1곳만 유료 API로 계산(모든 후보를 미리 계산하지 않음) | 완료 | `실사용 가능`(테스트 어댑터 기준) | 없음 |
| R16-4 실제 스크래핑 데이터 로컬 전용 가져오기 스크립트 신규 구현 | `scripts/import-bike-ports.mjs`(신규) | 명세 요구대로 **실제 포트 데이터는 공개 git 저장소·전달 코드 ZIP·정적 배포 자산에 절대 포함하지 않는다** — 운영자가 로컬에서만 실행하는 CLI로 분리, 실패(식별자 중복·필수값 누락·좌표 이상) 시 부분 반영 없이 전부 취소(트랜잭션). 사용자가 전달한 실제 후쿠오카 포트 스냅샷(1,075곳)으로 임시 DB(`/tmp`, 커밋 안 함)에 실제 가져오기를 실행해 1,075곳 전부와 메타데이터가 정확히 들어감을 확인 — **이 실제 데이터는 저장소에도, 전달 코드 ZIP에도 포함되지 않음**. 테스트 픽스처는 전부 합성 좌표만 사용 | 완료 | `실제 데이터로 실행 검증`(로컬 1회성, 미커밋) | 실제 운영 DB에 반영하려면 운영자가 스냅샷 파일을 로컬에 두고 이 스크립트를 직접 실행해야 함 — 데이터 상업적 재사용 조건이 아직 불확실하므로 본인 개발/테스트 용도로만 사용하고, 외부 베타 참가자에게는 실제 포트 데이터가 아니라 공식 지도 링크만 노출됨 |
| R16-5 화면 연결 — 진입 버튼·출발지 확인·후보→안내→반납 흐름 | `src/design/bike-ports.js`(신규), `src/design/index.html`, `src/design/spots.js` | 장소 상세에서 활성 지역(공급자 상태를 서버에서 실제로 조회해 확인된 지역만) + 확정 좌표일 때만 "자전거로 가기" 버튼 노출. 출발지가 없으면 "내 주변"으로 얼버무리지 않고 명시적으로 물어봄(GPS/직접입력, 평균 위치 폴백 없음 — 신규 `daBikeOriginResolver`). 진행 상태(목적지·고른 포트·단계)는 계정별 저장 데이터에 들어가 새로고침·외부 앱 전환에도 유지되고, 로그아웃 시 지워져 다른 계정으로 새지 않음(실제 Chromium 테스트로 확인). "반납했어요"는 서버가 자동 감지하지 않고 사용자가 직접 눌러야만 전환됨. 공식 지도 링크만 노출하고 커스텀 앱 URL 스킴은 쓰지 않음 | 완료 | `실사용 가능`(테스트 어댑터·합성 데이터 기준, Chromium으로 화면까지 실제 확인) | 없음 |
| R16-6 최종 회귀 + 관련 기존 화면 재확인 | 본 문서 | 신규 서버 테스트 2개(전체 통과) + 신규 Chromium E2E 1개(전체 통과, 콘솔/런타임 오류 0건) 확인 후, 이번에 공용 코드(`daFinishLogin`, 부팅 시퀀스, `daRequestMyLocation`/`manualLocationSheet` 시그니처)를 건드렸으므로 무관한 전체 회귀 대신 관련 기존 테스트만 재실행 — 그 과정에서 `#useMyLocationBtn`의 `onclick`에 함수를 직접 대입해 클릭 이벤트 객체가 콜백 인자로 잘못 전달되던 실제 회귀(`onReady is not a function`)를 재현·발견해 즉시 수정, 재실행으로 재확인(`scripts/test-location-optin.mjs`, `scripts/test-test-location-mode.mjs` 전체 통과) | 완료 | `실사용 가능` | 없음 |

**아직 미검증(정직하게 남김)**: 이 세션의 아웃바운드 네트워크가 막혀
있어 Google Routes BICYCLE 모드를 실제 공급자로 확인 못 함(모의
시뮬레이션까지만 검증), 실제 차리차리 반납 포트 운영 화면·실기기
확인, 실제 후쿠오카 현지에서 자전거로 이동하며 안내가 맞는지 확인,
스크래핑 데이터의 상업적 재사용 조건(운영자 확인 전) — 전부 사용자가
실제 기기·현지·계약 확인으로 진행해야 하는 다음 단계다. 가격
(9,900원/30일)·이용권 수량(무료 10곳+코스1회, 유료 50곳+코스30회)은
이번 라운드에서 전혀 바꾸지 않았다.

## -7. 15차(4차 후속) 갱신 요약

ChatGPT가 2차 독립검토를 통과시키고 "코드 재검토를 반복하기보다 실제
연결·휴대폰 테스트로 넘어가라"고 지시했다. 이번엔 문서를 늘리는 대신
**실제로 동작하는 코드**를 완성하는 데 집중했다 — 백업 스크립트,
테스트 모드 배너, 우선순위·베타 흐름 재정리(문서는 최소 수정).

| 항목 | 수정 위치 | 내용 | 완료 여부 | 상태 | 다음 행동(사용자) |
|---|---|---|---|---|---|
| R15-9 DB 온라인 백업 스크립트 신규 구현 | `scripts/backup-db.mjs`(신규) | 예전엔 외부 `sqlite3` CLI 설치를 전제로 한 셸 명령만 문서에 있었는데, Node.js 내장 `node:sqlite`의 백업 API(`backup()`)만으로 동작하는 스크립트를 새로 만들었다 — 서버가 그 DB 파일에 계속 쓰기 중이어도 안전한 온라인 백업. 실제로 스키마 테이블 전체(33개)가 백업 파일에 그대로 복사됨을 확인, 이미 있는 백업 파일을 실수로 덮어쓰지 않는 안전장치도 확인 | 완료 | `실사용 가능` | 크론 등으로 주기 실행 예약 |
| R15-10 서비스 테스트 모드 배너 신규 구현 | `src/design/index.html`, `src/design/spots.js` | "개발용 테스트 어댑터가 외부 베타에서 실제 서비스처럼 보이지 않게 하라"는 지시 반영 — 예전엔 서버가 테스트 모드(`/api/health`의 `testMode:true`, 실키 미설정)로 떠 있어도 화면에 아무 표시가 없어서, 베타 참가자가 로그인 코드를 요청해도 실제 메일이 영원히 안 오는데 그냥 멈춘 것처럼 보일 수 있었다. 앱을 열 때 서버 상태를 한 번 확인해 테스트 모드면 "⚠️ 이 서버는 테스트 모드입니다 — 로그인 코드·결제·장소조회가 전부 가짜(모의) 응답입니다"라는 배너를 화면 위에 계속 띄운다. 실제 Chromium으로 testMode true/false 두 경우 모두 배너 표시 여부가 정확함을 확인, 기존 회귀 테스트 중 콘솔/런타임 오류를 엄격히 확인하는 시나리오(`test-consumer-flow-e2e.mjs`, `test-google-signin-configured.mjs`) 재실행 — 새 부팅 시 호출이 추가돼도 전부 그대로 통과 | 완료 | `실사용 가능` | 없음 |
| R15-11 우선순위 체크리스트를 Google 로그인 최우선으로 재정렬 | `docs/OPERATIONS_SETUP.md` 6절 | "실제 로그인은 Google 로그인 우선, 이메일은 대체 경로"라는 지시대로 순서를 이메일·결제 우선에서 **Google 로그인(+이메일 대체 경로 동시 설정) → Places/Routes → 토스 테스트결제 → 날씨 → 베타 배포 → AI(그대로 최후)** 순으로 재정렬. 기존 비용 제한·가격(9,900원/30일)·이용권 수량은 손대지 않음 | 완료 | 문서 작업 | 6절 순서대로 진행 |
| R15-12 휴대폰 테스트 흐름을 한 흐름으로 통합 | `docs/RELEASE_STATUS.md` | 예전엔 "전체 흐름"과 "후쿠오카 가상위치 테스트"가 별개 단계로 분리돼 있었는데, 지시대로 **가져오기 → 분류·검색 → 테스트 위치(후쿠오카) 지정 → 실제 장소확인·코스생성 → 저장·재로그인 → 테스트 결제** 한 흐름으로 통합(가상위치·실GPS 구분은 그대로 유지). 운영자 본인 iPhone 우선 → 베타 참가자 Android 순서, 새 기기 구매 제안 없음도 유지 | 완료 | 문서 작업 | 1단계부터 순서대로 진행 |

**아직 미검증(정직하게 남김)**: 실제 Windows PC에서의 프로세스 실행,
실제 Resend·토스페이먼츠·Google Places/Routes·Google 로그인 키 연결,
실제 Google 계정으로 소유확인 화면을 끝까지 통과하는 것, 실제
iPhone/Android 기기 확인, 실제 AI 공급자 청구 비용 대조 — 전부
사용자가 실제 키·기기·환경으로 진행해야 하는 다음 단계다.

## -6. 15차 재검토(2차 독립검토 후속) 갱신 요약
...
**실기기 베타 확인 절차(15차, 4차 후속 갱신 — 한 흐름으로 통합, 별도
기기 구매·유료 테스트 서비스 계약 없이 진행) — 아래 순서로, 1단계
(운영자 본인 iPhone)부터 먼저 끝까지 확인한 뒤에 2단계(베타 참가자)로
넘어간다:**

**1단계 — 운영자 본인의 iPhone(Safari)에서 전체 흐름 한 번에 끝까지**
아래 순서를 끊지 말고 하나로 이어서 확인한다(중간에 막히는 지점이
있으면 그 화면 이름을 그대로 적어 둔다):
1. 본인 Google 지도 저장 목록(Takeout) 가져오기
2. 자동 분류 결과 확인 · 검색
3. **테스트 위치를 후쿠오카로 지정**(실제로 후쿠오카에 가지 않고도
   코스·거리순 로직을 확인하려면, 관리자 전용 API로 그 계정에 테스트
   위치를 켠 뒤 — `OPERATIONS_SETUP.md` 1-6b절,
   `/api/admin/test-access` — 화면에서 후쿠오카 좌표를 테스트
   ...
   반드시 배너로 구분돼야 함).
4. 실제 장소 위치확인(좌표 없는 장소 하나) · 코스 생성
5. 저장 → 로그아웃 → 재로그인(저장한 게 그대로 남아 있는지)
6. 테스트 결제(완료 후 토스 쪽에서 반드시 취소 처리)
7. 마지막에 테스트 위치 배너의 "실제 위치로 되돌리기"로 반드시
   끈다 — 켠 채로 실사용자 계정에 남기지 않는다.

여유가 되면 일본어 목적지에서 "여기서 쓸 말" 듣기(🔊)·글자 크게
보기도 같은 흐름 중에 함께 확인한다. 위치 권한을 요청하는 "내 위치
기준" 버튼은 이 흐름과 별개로 **실제 GPS**를 쓴다는 점을 참가자
안내에도 그대로 남긴다(아래 2단계 3번 참고) — 위 3번의 가상위치와는
다른 경로다.

**2단계 — 소수 베타 참가자(참가자 본인 기기, 5~10분 분량)**
...
3. 거리순 정렬 옆 "내 위치 기준" 버튼 → 위치 권한 허용/거절 각각 확인(이건 실제 GPS다 — 1단계의 가상위치와 다른 경로)
...
에뮬레이션이거나 모의(mock) 응답 기반 검증이다(R12-5 등) — 위 1~2단계처럼
