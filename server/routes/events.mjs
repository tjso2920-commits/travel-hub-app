...
const EVENT_SCHEMA = {
  ...
  // 2026-09-11 재검토(9차) 6-4절 — 가입→가져오기→코스완성→실사용→피드백
  // 퍼널 측정. 나머지 3단계는 이미 있던 이벤트(import_result/
  // course_generated)로 충분해 새로 안 만들었다(중복 방지).
  signup_completed: { props: [] }, // 새 계정이 실제로 만들어진 순간(로그인 재사용 아님)
  place_visited: { props: [] }, // "실사용" 신호 — 실제로 방문 완료 표시를 한 순간(장소명 등은 안 실음)
...
const ALLOWED_FEEDBACK_TYPES_FOR_EVENT = new Set(['import', 'location-route', 'usage', 'payment', 'other']);
...
function validateProps(name, props) {
  ...
  if (name === 'feedback_submitted' && !ALLOWED_FEEDBACK_TYPES_FOR_EVENT.has(p.type)) return { ok: false, reason: 'invalid-type' };
  if (name === 'survey_submitted' && typeof p.actually_traveled !== 'boolean') return { ok: false, reason: 'invalid-actually-traveled' };
