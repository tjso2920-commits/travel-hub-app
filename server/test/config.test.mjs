...
// 사고가 났었다 — 재현하지 않는지 확인) ---
{
  ...
  t('결제 키만 있음 — payment는 real', c.services.payment === 'real');
  t('결제 키만 있음 — placeLookup은 여전히 test(다른 서비스 영향 안 받음)', c.services.placeLookup === 'test');
  t('결제 키만 있음 — email도 여전히 test', c.services.email === 'test');
  t('결제 키만 있음 — 전체 testMode 요약값은 false(하나라도 real이면 false)', c.testMode === false);
}
...
// --- 셋 다 있으면 전부 real, testMode 요약값 false ---
{
  const c = buildConfig({ GOOGLE_PLACES_API_KEY: 'k', PAYMENT_PG_SECRET: 's', EMAIL_API_KEY: 'e' });
  t('셋 다 있음 — 전부 real', c.services.placeLookup === 'real' && c.services.payment === 'real' && c.services.email === 'real');
  t('셋 다 있음 — 전체 testMode 요약값 false', c.testMode === false);
}

// --- FORCE_TEST_MODE=true는 키가 있어도 전부 test로 강제한다(로컬
// 개발·테스트 실행 중 실수로 남은 실제 키를 안전하게 무시) ---
{
  const c = buildConfig({ GOOGLE_PLACES_API_KEY: 'k', PAYMENT_PG_SECRET: 's', EMAIL_API_KEY: 'e', FORCE_TEST_MODE: 'true' });
