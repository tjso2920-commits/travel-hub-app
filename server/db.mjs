'use strict';
/**
 * SQLite 기반 저장소(node:sqlite, Node.js 내장 모듈 — 별도 설치 불필요).
 * 2026-09-12 재검토(15차, 후속) — 이 세션이 실제로 실행해 확인한
 * 버전은 Node.js 24다(별도 플래그 없이 동작). package.json의
 * `engines.node`도 이 기준으로 맞춰 뒀다 — 더 낮은 버전에서의 동작
 * 여부는 이 세션이 확인하지 않았다.
 *
